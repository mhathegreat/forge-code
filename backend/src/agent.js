...
const { OPENROUTER_API_KEY, MODEL } = require('./config');
const { toolDefs, execTool } = require('./tools');
const { buildSystemPrompt } = require('./systemPrompt');
const { listTree, projectDir } = require('./files');
...
async function streamCompletion(messages, emit) {
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: {
      Authorization: 'Bearer ' + OPENROUTER_API_KEY,
      'Content-Type': 'application/json',
      'HTTP-Referer': 'http://192.168.4.21:3001',
      'X-Title': 'KimiStudio',
    },
    body: JSON.stringify({
      model: MODEL,
...
// emit: function(event) -> send to client
// Returns the final assistant text.
async function runAgent({ projectId, userMessage, history = [], meta = {}, emit = () => {} }) {
  const tree = await listTree(projectId);
  let agents = '';
  try { agents = await fsp.readFile(path.join(projectDir(projectId), 'AGENTS.md'), 'utf8'); } catch {}

  const messages = [{ role: 'system', content: buildSystemPrompt({ projectId, meta, agents, tree }) }];
  ...
  for (let iter = 0; iter < MAX_ITERS; iter++) {
    ...
    let result;
    try {
      result = await streamCompletion(messages, emit);
    } catch (e) {
    ...
    for (const tc of toolCalls) {
      ...
      let out;
      try {
        out = await execTool(projectId, tc.function.name, args, emit);
      } catch (e) {
        out = 'ERROR: ' + e.message;
      }

      emit({ type: 'tool_result', id: tc.id, name: tc.function.name, result: truncate(out, 4000) });
