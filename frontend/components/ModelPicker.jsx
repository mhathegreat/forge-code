...
import { X, Search, Check, Loader2, Zap, CornerDownLeft } from 'lucide-react';

function fmtPrice(m) {
  if (!m.pricingKnown) return null;
...
export default function ModelPicker({ data, loading, error, current, onSelect, onClose }) {
  ...
  const anyPricing = models.some((m) => m.pricingKnown);

  const filtered = useMemo(() => {
    let list = models;
    if (freeOnly && anyPricing) list = list.filter((m) => m.free);
    const s = q.trim().toLowerCase();
    if (s) list = list.filter((m) => m.id.toLowerCase().includes(s) || m.name.toLowerCase().includes(s));
    return list.slice(0, 120);
  }, [models, q, freeOnly, anyPricing]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center" onClick={onClose}>
      <div
        ...
        className="w-[560px] max-h-[75vh] bg-ink-900 border border-ink-700 rounded-xl shadow-2xl flex flex-col"
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-ink-700">
          <div className="text-sm font-semibold">
            Choose model
            {provider && <span className="text-gray-500 font-normal text-xs ml-2">via {provider.name}</span>}
          </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          ...
          {!loading && error && (
            <div className="text-center text-amber-400/90 text-xs px-8 py-8">
              {error}
            </div>
          )}
        ...
        <div className="px-4 py-2.5 border-t border-ink-700 space-y-1.5">
          <div className="flex items-center gap-2">
            <input
              value={manual}
              onChange={(e) => setManual(e.target.value)}
              onKeyDown={(e) => { if (e.key === 'Enter') useManual(); }}
              placeholder="…or type a model id manually (e.g. gpt-5.2, claude-opus-4-8)"
              className="flex-1 bg-ink-800 border border-ink-700 rounded-lg px-2.5 py-1.5 text-xs font-mono outline-none focus:border-accent"
            />
            <button
              onClick={useManual}
              disabled={!manual.trim()}
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs bg-ink-800 hover:bg-ink-750 border border-ink-700 rounded-lg text-gray-300 disabled:opacity-40"
            >
              <CornerDownLeft size={11} /> Use
            </button>
          </div>
          <div className="text-[10px] text-gray-600">
            Applies to this project. Change provider in Settings (⚙️). Free-tier models can be rate-limited upstream.
          </div>
        </div>
      </div>
    </div>
